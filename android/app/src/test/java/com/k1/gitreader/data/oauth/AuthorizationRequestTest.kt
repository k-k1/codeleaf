...
class AuthorizationRequestTest {
    ...
    fun buildIncludesRequiredParams() {
        val req = BitbucketAuthorization.build("cid123", "gitreader://oauth", "st-XYZ")
        ...
        assertTrue(req.url.contains("redirect_uri=gitreader%3A%2F%2Foauth"))
        assertEquals("st-XYZ", req.state)
        assertEquals("gitreader://oauth", req.redirectUri)
    }
